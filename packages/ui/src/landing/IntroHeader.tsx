...
export default function IntroHeader({
  ...
  return (
    <>
          ...
          <nav className="pointer-events-none absolute inset-0 hidden items-center justify-center lg:flex">
            {INTRO_NAV.map(({ label, path }) => (
              <button
                ...
                className={`pointer-events-auto flex h-15 cursor-pointer items-center border-b-3 px-5 text-[17px] font-medium transition-colors
                  ${
                    pathname === path
                      ? "border-yellow-600"
      ...
      {isOpen && (
        <div
          className="fixed inset-0 z-[90] bg-black/5 lg:hidden"
          onClick={() => setIsOpen(false)}
        />
      )}
      <div
        className={`fixed right-0 top-15 z-[100] h-[calc(100vh-60px)] w-72 border-l border-gray-200 bg-white shadow-xl transition-all duration-300 ease-out lg:hidden
