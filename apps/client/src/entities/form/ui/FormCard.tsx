...
export default function FormCard({ form, onOpen }: FormCardProps) {
  ...
  return (
    <div className="flex h-20 w-[800px] shrink-0 items-center rounded-xl bg-white pr-[64px] pl-4 shadow transition-shadow duration-200 hover:shadow-md">
      {submitted ? (
      ...
      {closed ? (
        ...
        <button
          type="button"
          onClick={() => onOpen(id)}
          className={`ml-[72px] flex h-8 w-20 shrink-0 cursor-pointer items-center justify-center rounded-xl border font-medium transition-all duration-150 hover:shadow-sm active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-yellow-600/50 ${
            submitted
